...
class English(WikiFeed):
    ...
    def items(self):
        ...
        return Page.objects.filter(lang=en).exclude(pubdate=None).order_by('-pubdate')[:NUMBER_OF_ITEMS]
...
class Portuguese(WikiFeed):
    ...
    def items(self):
        ...
        return Page.objects.filter(lang=pt).exclude(pubdate=None).order_by('-pubdate')[:NUMBER_OF_ITEMS]
